...
//Shortest burst to the highest burst.
...
void add(char *name, int priority, int burst){
    ...
    if(i == 1) {
        ...
        printf("The file is read through and add() is called based on the specified scheduler algorithm: "
               ...
               "burst values to place in order.\nSchedule() is called from "
    ...
    printf("Creating linked list. Inserting the nodes in order of smallest burst to largest.\n\n");
    ...
    if(head==NULL){
        ...
        temp = head;
        //in the case of the head task being greater than the newTask
        // 20 -> 10 want newNode to be new head
        if(temp->task->burst >= newNode->task->burst){
            newNode->next = head;
            head = newNode;
        }else {
            while (temp->next != NULL && temp->next->task->burst < newNode->task->burst) {//traverse
                temp = temp->next;
            }
